    ...
    try:
        match cmd:
            ...
            case "ask":
                ...
                boxes = boxes | {temp[-1]: input(get_arg(0, args, boxes) + " : ")}
            case "del":
                del boxes[get_arg(0, args, boxes)]
            case "test":
                if test(get_arg(0, args, boxes), get_arg(1, args, boxes), get_arg(3, args, boxes)):
                    boxes = boxes | {get_arg(0, args, boxes): get_arg(4, args, boxes)}
                else:
                    boxes = boxes | {get_arg(0, args, boxes): get_arg(5, args, boxes)}
            case "math":
                boxes = boxes | {get_arg(0, args, boxes): str(math(get_arg(1, args, boxes),
        ...
        print(Back.RED + Fore.WHITE + "at line : " + str(l) +
              "  " + str(undo_mk([command])) +
              " boxes : " + str(boxes) +
              " marks : " + str(marks))
